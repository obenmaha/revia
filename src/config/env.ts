...
function getRuntimeConfig(): Partial<EnvConfig> {
  ...
  if (typeof window !== 'undefined' && window.__APP_CONFIG__) {
    try {
      ...
      if (runtimeConfig && runtimeConfig.supabase && runtimeConfig.supabase.url) {
      ...
      console.warn('⚠️  Erreur lors du chargement de la config runtime:', error);
    }
  }
...
export const validateEnv = (): boolean => {
  try {
    ...
    if (import.meta.env.VITE_SUPABASE_SERVICE_ROLE_KEY) {
      ...
      console.error('Utilisez-la uniquement dans Edge Functions ou scripts backend.');
