...
✅ **READY FOR REVIEW** - Corrections appliquées
...
const sessionSchema = z.object({
  ...
  date: z.date()
    ...
    .max(new Date(Date.now() + 365 * 24 * 60 * 60 * 1000), "Date trop éloignée"),
...
describe('SessionForm Validation', () => {
  it('should validate session name correctly', () => {
    const validData = { name: 'Séance matin', date: new Date('2024-12-25'), type: 'rehabilitation' };
  ...
  it('should reject invalid session name', () => {
    ...
    expect(result.error?.issues[0].message).toBe('Nom requis (min 3 caractères)');
  });

  it('should validate date constraints', () => {
    const pastDate = { name: 'Séance', date: new Date('2020-01-01'), type: 'rehabilitation' };
